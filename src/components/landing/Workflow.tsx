...
export const Workflow: React.FC = () => {
  ...
  return (
    <section className="py-20 bg-slate-100/60 border-t border-b border-slate-200/80 relative overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="text-center mb-16">
          <span className="text-xs font-bold text-indigo-700 font-mono uppercase tracking-widest px-3 py-1 rounded-full bg-indigo-50 border border-indigo-200 mb-3 inline-block">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 relative">
          {steps.map((step, idx) => {
            ...
            return (
              <div
                key={idx}
                className="bg-white border border-slate-200 hover:border-indigo-300 rounded-2xl p-6 relative transition-all hover:shadow-xl hover:shadow-indigo-500/10 group"
              >
                <div className="flex items-center justify-between mb-4">
                  <div className={`w-12 h-12 rounded-xl bg-gradient-to-br ${step.color} p-0.5 shadow-md`}>
                ...
                <span className="text-xs font-mono text-indigo-600 block mb-3 font-semibold">{step.subtitle}</span>
